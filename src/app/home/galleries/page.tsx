...
export default function page() {
  ...
  //Tag
  const [stateTag, dispatchTag] = useReducer(tagReducer, initialTagState);
  const { tags } = stateTag;
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.log("DataTag", response.data.result);
        if (isMounted) {
          dispatchTag({
            type: TAG_ACTIONS.FETCH_SUCCESS,
        ...
        console.error("Lỗi khi lấy dữ liệu từ API:", error);
        if (isMounted) {
          dispatchTag({ type: TAG_ACTIONS.FETCH_ERROR });
        }
      }
    };
